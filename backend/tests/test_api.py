...
def test_scan_repository_and_report_workflow():
    ...
    # Get
    fetched = scan_repository.get("SCAN-TEST-V2-01")
    assert fetched is not None
    assert fetched.filename == "test_acoustic.png"

    # Report
    report_res = client.get("/api/scans/SCAN-TEST-V2-01/report")
    ...
    # Delete
    del_res = client.delete("/api/scans/SCAN-TEST-V2-01")
    assert del_res.status_code == 200
    assert scan_repository.get("SCAN-TEST-V2-01") is None
