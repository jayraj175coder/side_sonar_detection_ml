...
const StatCard: React.FC<{ label: string; value: string; sub?: string; color?: string }> = ({
  ...
  <div className="p-3.5 sm:p-4 rounded-xl bg-[#060D17] border border-[#152438] transition-all hover:border-[#4CD9E8]/40">
    <p className="text-[9px] font-mono text-[#7C8AA0] uppercase tracking-widest truncate">{label}</p>
    <p className="text-xl sm:text-2xl font-mono font-black mt-1 truncate" style={{ color }}>{value}</p>
    {sub && <p className="text-[9px] font-mono text-[#7C8AA0] mt-0.5 truncate">{sub}</p>}
  </div>
);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto font-sans select-none px-2 sm:px-4">
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-[#060D17] border border-[#152438] flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xl">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-mono font-black text-[#EAEFF5] tracking-tight">Mission Analytics & Telemetry</h1>
            <span className={`px-2 py-0.5 rounded text-[9px] font-mono font-bold ${isBackendConnected ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' : 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40'}`}>
              {isBackendConnected ? 'LIVE ONNX DATA' : 'SURVEY ACTIVE'}
            </span>
          </div>
          <p className="text-xs font-mono text-[#7C8AA0] mt-1">
            {MISSION_DATA.id} — {MISSION_DATA.name} · {MISSION_DATA.region}
          </p>
        </div>
        <button
          onClick={handleExport}
          className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#0A1322] border border-[#152438] hover:border-[#4CD9E8]/40 text-[#4CD9E8] text-xs font-mono font-bold transition-all cursor-pointer shadow-md"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5 sm:gap-3">
        <div className="col-span-2"><StatCard label="Surveyed Area" value={`${staticA.surveyedArea} km²`} sub="EEZ Hydrographic Grid" color="#4CD9E8" /></div>
        <div className="col-span-2"><StatCard label="Live Ingested Scans" value={String(dynamicMetrics.totalScansCount)} sub="Active Waterfall Swaths" color="#29B6F6" /></div>
        <div className="col-span-2"><StatCard label="Total Detections" value={String(dynamicMetrics.totalDetectionsCount)} sub="Confirmed Underwater Targets" color="#3FD98A" /></div>
        <StatCard label="Avg Conf" value={`${dynamicMetrics.avgConfidence.toFixed(1)}%`} sub="Acoustic Fit" color="#00D4AA" />
        <StatCard label="Ghost Nets" value={String(dynamicMetrics.ghostNetCount)} sub="Priority 1 ALDFG" color="#A855F7" />
      </div>

      {/* Second KPI Strip */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
        <StatCard label="Seafloor Depth" value={`${staticA.avgDepth} m`} sub="Continental Shelf Mean" color="#29B6F6" />
        <StatCard label="Track Length" value={`${staticA.trackLength} km`} sub="AUV Survey Distance" color="#F5A623" />
        <StatCard label="False Positives Cut" value="92%" sub="Acoustic Shadow Verification" color="#3FD98A" />
        <StatCard label="ONNX Transducer" value={MISSION_DATA.frequency} sub="900 kHz CHIRP Side-Scan" color="#4CD9E8" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-[#060D17] border border-[#152438] shadow-lg">
          <SectionHeader icon={<Activity className="w-4 h-4 text-[#4CD9E8]" />} title="Detections Over Survey Track" sub="Cumulative contacts per hydrographic transect interval" />
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={staticA.detectionsOverTime}>
              <defs>
                <linearGradient id="detGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#4CD9E8" stopOpacity={0.25} />
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-[#060D17] border border-[#152438] shadow-lg">
          <SectionHeader icon={<BarChart2 className="w-4 h-4 text-[#29B6F6]" />} title="Confidence Score Distribution" sub="Target count across neural probability tiers" />
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={dynamicMetrics.confDist}>
              ...
              <Bar dataKey="count" name="Targets" radius={[4, 4, 0, 0]}>
                {dynamicMetrics.confDist.map((_: { range: string; count: number }, i: number) => (
                  <Cell key={i} fill={i === 0 ? '#00D4AA' : i === 1 ? '#38BDF8' : i === 2 ? '#F59E0B' : '#EF4444'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-[#060D17] border border-[#152438] shadow-lg">
          <SectionHeader icon={<PieIcon className="w-4 h-4 text-[#F5A623]" />} title="Target Taxonomy Breakdown" sub="Classification distribution of verified contacts" />
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1 w-full space-y-1.5">
              {dynamicMetrics.classDist.map(({ name, count, color }: { name: string; count: number; color: string }) => (
                <div key={name} className="flex items-center justify-between text-[11px] font-mono p-1 rounded hover:bg-[#0A1322]">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full shadow-sm" style={{ background: color }} />
                    <span className="text-[#E0F7F4] font-medium">{name}</span>
                  </div>
        ...
        {/* 4. Depth breakdown */}
        <div className="p-4 sm:p-5 rounded-2xl bg-[#060D17] border border-[#152438] shadow-lg">
          <SectionHeader icon={<Layers className="w-4 h-4 text-[#29B6F6]" />} title="Seafloor Depth Strata" sub="Target contacts by bathymetric depth band" />
          <div className="space-y-3">
            {staticA.depthDistribution.map(({ range, count }) => (
              <div key={range} className="space-y-1">
                <div className="flex justify-between text-[10px] font-mono">
                  <span className="font-bold text-[#4CD9E8]">{range}</span>
                  <span className="text-[#7C8AA0]">{count} targets ({Math.round((count / staticA.totalTargets) * 100)}%)</span>
                </div>
                <div className="w-full h-2 bg-[#03070E] rounded-full overflow-hidden border border-[#102436]">
                  <div className="h-full rounded-full bg-gradient-to-r from-[#00D4AA] to-[#38BDF8]" style={{ width: `${(count / staticA.totalTargets) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
